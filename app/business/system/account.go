...
func (api *Account) GetList(c *gf.GinCtx) {
	...
	list, err := MDB.Fields("id,status,name,username,avatar,tel,mobile,email,dept_id,remark,city,address,company,createtime").
	...
	if err != nil {
		...
		for _, val := range list {
			...
			//头像
			if val["avatar"] == nil {
				val["avatar"] = gvar.New(gf.GetLocalUrl() + "/resource/uploads/static/avatar.png")
			} else {
				val["avatar"] = gvar.New(gf.GetFullUrl(val["avatar"].String()))
			}
		}
