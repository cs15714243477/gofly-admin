...
func (api *Attachment) Del(c *gf.GinCtx) {
	param, _ := gf.RequestParam(c)
	fileList, _ := gf.Model("business_attachment").WhereIn("id", param["ids"]).Fields("id,url").Select()
	...
	if err != nil {
		...
		for _, val := range fileList {
			uploads.DelFile(val["url"].String())
		}
		gf.Success().SetMsg("删除文件成功！").SetData(res2).SetExdata(fileList).Regin(c)
	}
}
