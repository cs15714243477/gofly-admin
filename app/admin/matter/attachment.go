...
func (api *Attachment) GetList(c *gf.GinCtx) {
	...
	MDB := gf.Model("admin_attachment").Where(whereMap)
	...
	if err != nil {
		...
		for _, val := range list {
			if _, ok := val["url"]; ok && val["url"].String() != "" && !strings.Contains(val["url"].String(), "http") {
				val["url"] = gvar.New(gf.GetFullUrl(val["url"].String()))
			}
		}
...
func (api *Attachment) Del(c *gf.GinCtx) {
	param, _ := gf.RequestParam(c)
	file_list, _ := gf.Model("admin_attachment").WhereIn("id", param["ids"]).Array("url")
