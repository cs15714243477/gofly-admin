	...
	if err != nil {
		...
		for _, val := range list {
			if _, ok := val["image"]; ok && val["image"].String() != "" && !strings.Contains(val["image"].String(), "http") {
				val["image"] = gvar.New(gf.GetFullUrl(val["image"].String()))
			}
		}
...
func (api *Attachment) Save(c *gf.GinCtx) {
	...
	if f_id == 0 {
		...
		getcount, _ := gf.Model("admin_attachment").Where("pid", param["pid"]).Where("title like ?", fmt.Sprintf("%s%v%s", "%", param["title"], "%")).Count()
		param["title"] = fmt.Sprintf("%s%v", param["title"], getcount+1)
		addId, err := gf.Model("admin_attachment").Data(param).InsertAndGetId()
		if err != nil {
			gf.Failed().SetMsg("添加失败").Regin(c)
		} else {
			//更新排序
			gf.Model("admin_attachment").Data(map[string]interface{}{"weigh": addId}).Where("id", addId).Update()
			getdata, _ := gf.Model("admin_attachment").Where("id", addId).Fields("id,pid,name,title,type,url,filesize,mimetype,storage").Select()
			gf.Success().SetMsg("添加成功！").SetData(getdata).Regin(c)
		}
	} else {
		res, err := gf.Model("admin_attachment").
			Data(param).
			Where("id", f_id).
			Update()
		if err != nil {
...
func (api *Attachment) Del(c *gf.GinCtx) {
	param, _ := gf.RequestParam(c)
	file_list, _ := gf.Model("admin_attachment").WhereIn("id", param["ids"]).Where("type != ?", 1).Array("url")
	res2, err := gf.Model("admin_attachment").WhereIn("id", param["ids"]).Delete()
	if err != nil {
...
func (api *Attachment) DelDir(c *gf.GinCtx) {
	param, _ := gf.RequestParam(c)
	res2, err := gf.Model("admin_attachment").Where("id", param["id"]).Delete()
	if err != nil {
		gf.Failed().SetMsg("删除失败").Regin(c)
	} else {
		delCycle(param["id"])
...
func delCycle(id interface{}) {
	filedata, _ := gf.Model("admin_attachment").Where("pid", id).Fields("id,type").Select()
	if filedata != nil && len(filedata) > 0 {
		for _, val := range filedata {
			delCycle(val["id"])
			if val["type"].Int64() == 0 {
				file_path, _ := gf.Model("admin_attachment").Where("id", val["id"]).Value("url")
				if file_path != nil {
					gf.DelOneFile(file_path.String())
				}
			}
			gf.Model("admin_attachment").Where("id", val["id"]).Delete()
		}
	}
}
...
func (api *Attachment) GetMyFiles(c *gf.GinCtx) {
	...
	whereMap := gf.Model("admin_attachment").Where("business_id", user.BusinessID).Where("pid", pid)
	if searchword != "" {
	...
	if err != nil {
		...
		for _, val := range list {
			if _, ok := val["cover_url"]; ok && val["cover_url"].String() != "" && !strings.Contains(val["cover_url"].String(), "http") {
				val["cover_url"] = gvar.New(gf.GetFullUrl(val["cover_url"].String()))
			}
		}
		common_lisr, _ := gf.Model("admin_attachment").Where("pid", pid).Where("is_common", 1).Fields("id,pid,name,title,type,url,filesize,mimetype,storage,cover_url,is_common").Order("type desc,weigh desc,id desc").Select()
		if list != nil {
		...
		dirmenu, _ := gf.Model("admin_attachment").WhereIn("id", allids).Fields("id,pid,title").Select()
...
func getAllParentIds(id interface{}) []interface{} {
	...
	parent_id, _ := gf.Model("admin_attachment").Where("id", id).Value("pid")
	if parent_id != nil {
...
func (api *Attachment) UpImgPid(c *gf.GinCtx) {
	param, _ := gf.RequestParam(c)
	res2, err := gf.Model("admin_attachment").Where("id", param["imgid"]).Data(map[string]interface{}{"pid": param["pid"]}).Update()
	if err != nil {
