...
// api接口模版
type Api struct{ NoNeedAuths []string }

func init() {
	fpath := Api{NoNeedAuths: []string{"*"}}
	gf.Register(&fpath, fpath)
}

// 获取列表
func (api *Api) getList(c *gf.GinCtx) {
	pageSize := gconv.Int(c.DefaultQuery("pageSize", "10"))
	// 搜索添条件
	param, _ := gf.RequestParam(c)
	whereMap := gmap.New()
	businessID := c.GetInt64("businessID") //当前商户ID
	if gf.DbHaseField("business_user", "business_id") && businessID > 0 {
...
// 获取详情
func (api *Api) getDetail(c *gf.GinCtx) {
	id := c.DefaultQuery("id", "")
	if id == "" {
		gf.Failed().SetMsg("请传参数id").Regin(c)
	} else {
		data, err := gf.Model("business_user").Where("id", id).
			Fields("avatar,birthday,business_id,createtime").Find()
		if err != nil {
			gf.Failed().SetMsg("获取详情失败").SetData(err).Regin(c)
		} else {
			if data != nil {
				if _, ok := data["image"]; ok && !strings.Contains(data["image"].String(), "http") {
					data["image"] = gvar.New(gf.GetFullUrl(data["image"].String()))
				}
			}
...
// 保存
func (api *Api) save(c *gf.GinCtx) {
	param, _ := gf.RequestParam(c)
	var f_id = gf.GetEditId(param["id"])
	if f_id == 0 {
		if gf.DbHaseField("business_user", "business_id") {
			param["business_id"] = c.GetInt64("businessID") //当前用户商户ID
		}
		addId, err := gf.Model("business_user").Data(param).InsertAndGetId()
		if err != nil {
			gf.Failed().SetMsg("添加失败").SetData(err).Regin(c)
		} else {
			if addId != 0 {
				gf.Model("business_user").Data(gf.Map{"weigh": addId}).Where("id", addId).Update()
			}
			gf.Success().SetMsg("添加成功！").SetData(addId).Regin(c)
		}
	} else {
		res, err := gf.Model("business_user").Data(param).Where("id", f_id).Update()
		if err != nil {
...
// 删除
func (api *Api) del(c *gf.GinCtx) {
	param, _ := gf.RequestParam(c)
	res, err := gf.Model("business_user").WhereIn("id", param["ids"]).Delete()
	if err != nil {
