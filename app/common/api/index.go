...
// 5.测试配置
func (api *Index) GetTest(c *gf.GinCtx) {
	// appConf, _ := gcfg.Instance("upload").Get(c, "Type")
	tencentcos, _ := gcfg.Instance("upload").Get(c, "tencentcos")
	// appConf_arr := gconv.Map(appConf)
	appConfAll, _ := gcfg.Instance("upload").Data(c)
	gf.Failed().SetMsg("测试配置").SetData(tencentcos.Map()).SetExdata(appConfAll).Regin(c)
}
