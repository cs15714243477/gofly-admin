...
func (api *Index) Save(c *gf.GinCtx) {
	...
	db.Exec(string(sqls))
	// 4.2 安装admin端
	if formApp["isInstalladmin"] == "install" {
	...
	_, upberr := db.Exec("update "+table_prefix+"business_account set username = ?, password = ?, salt = ? where id = ?", formApp["username"], gf.Md5(businesspass), salt, 1)
	if upberr != nil {
	...
	//安装admin端
	if formApp["isInstalladmin"] == "install" {
		adminpass := fmt.Sprintf("%v%v", gf.Md5(formApp["adminPassword"].(string)), salt)
		_, upaerr := db.Exec("update "+table_prefix+"admin_account set username = ?, password = ?, salt = ? where id = ?", formApp["adminUsername"], gf.Md5(adminpass), salt, 1)
		if upaerr != nil {
	...
	os.Create(filePath)
	//8.安装前端页面
	if gf.IsEmpty(formApp["vueobjroot"]) {
		formApp["vueobjroot"] = filepath.Join(path, "/web")
	}
	//前端zip代码包
	//8.1 如果没有filepath文件目录就创建一个
	...
	//安装admin端
	if formApp["isInstalladmin"] == "install" { //解压admin
	...
	//9.修改应用配置
	upAppconf := gf.Map{
		"vueobjroot":   gstr.Replace(formApp["vueobjroot"].(string), "\\", "/"),
		"busDirName":   "business",
		"adminDirName": "admin",
	}
	...
	gform.SetConfig(gform.Config{
		"default": gform.ConfigGroup{
			gform.ConfigNode{
				Hostname:  gconv.String(formDB["hostname"]),
				Hostport:  gconv.String(formDB["hostport"]),
				Username:  gconv.String(formDB["username"]),
				Password:  gconv.String(formDB["password"]),
				Dbname:    gconv.String(formDB["dbname"]),
				Prefix:    table_prefix,
				Type:      "mysql",
				Role:      "master",
				Weight:    100,
