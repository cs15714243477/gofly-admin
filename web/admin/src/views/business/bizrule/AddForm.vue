...
<script lang="ts">
  ...
  //api
  import { getContent,getParent,getRoutes, save,RuleItem } from '@/api/business/bizrule';
     ...
     const handleSubmit = async () => {
      try {
          ...
          if (!res) {
            ...
            Message.loading({content:"更新中",id:"save",duration:0})
            await save(unref(formData));
            Message.success({content:"更新成功",id:"save",duration:2000})
            closeModal()
            emit('success');
            setLoading(false);
          }
          ...
          setLoading(false);
          Message.loading({content:"更新中",id:"save",duration:1})
        }
      };
      //切换菜单类型
