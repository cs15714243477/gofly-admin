                  ...
                  <template #extra="nodeData">
                    <span v-if="nodeData.key!=0" class="page-tree-tool" :class="{'page-tree-tool-active':nodeData.showbtn}">
                        <a-popover position="br" trigger="click" :popup-visible="nodeData.showbtn"  @popup-visible-change="(visible)=>nodeData.showbtn=visible">
                          ...
                          <template #content>
                            <div class="tigmenu">
                              <div class="item" @click="handleEditCate(nodeData)"><Icon class="bicon" icon="svgfont-edit-o" :size="13"></Icon> 编辑</div>
                              <div class="item delete" @click="handleDelCate(nodeData.id)"><Icon class="bicon" icon="svgfont-delete-o" :size="13"></Icon> 删除</div>
                            </div>
                          </template>
                        </a-popover>
                ...
                <div class="search-field">
                  <div class="search-line search-drown" :class="{searchdowncss:searchdown}">
                    <div class="search-drown-box" ref="searchRef">
                      <a-space wrap>
								<a-input style="width: 120px;" v-model="formModel.title" placeholder="输入标题" allow-clear />
								<FormDicSelect style="width: 120px;" v-model="formModel.userType" placeholder="输入用户类型" dicgroupid="2"/>
								<a-input style="width: 120px;" v-model="formModel.status" placeholder="输入状态" allow-clear />
								<span>上传时间：<a-range-picker style="width: 230px;" v-model="formModel.createtime" allow-clear :shortcuts="shortcuts" shortcuts-position="left" @change="fetchData"/></span>
                      </a-space>
                    </div>
                  </div>
                </div>
              ...
              <div class="table-container">
                ...
                >
						...
						<template #status="{record,column}">
							<dot-status :value="record[column.dataIndex]" :dict="[{value:0,label:'正常',color:'primary'},{value:1,label:'隐藏',color:'success'}]"/>
						</template>
