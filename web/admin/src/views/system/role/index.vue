    ...
    <page-card breadcrumb scrollPage :isFullscreen="isFullscreen">
      ...
      >
        ...
        <template #status="{ record }">
          <a-switch v-if="record.rules!='*'&&(record.pid==0||record.pid!=max_pid)" type="round" v-model="record.status" :checked-value="0" :unchecked-value="1" @change="handleStatus(record)">
              <template #checked>
                开
              </template>
              <template #unchecked>
        ...
        <template #operations="{ record }">
          <template v-if="record.rules!='*'&&(record.pid==0||record.pid!=max_pid)">
