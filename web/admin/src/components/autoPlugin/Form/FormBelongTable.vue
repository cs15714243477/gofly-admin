...
<template>
  <a-select :model-value="modelValue"  :placeholder="placeholder" allow-clear allow-search 
  ...
  watch(props, (nweProps)=>{
      if(nweProps.tablename&&props.tablename!=nweProps.tablename){
        handleCommonSelect(nweProps.tablename)
      }
  })
  ...
  const handleCommonSelect=async(tablename:string)=>{
      ...
      const mlist= await getTableDataForm({tablename:tablename,showfield:props.showfield})
