<template>
  <BasicModal v-bind="$attrs" @register="registerModal" :isPadding="false" :loading="loading" width="998px" @height-change="onHeightChange" :minHeight="modelHeight" :title="getTitle" @ok="handleSubmit">
                    ...
                    <div class="file_item" v-for="iten in myFileList">
                      <div>
                        <div :class="{imgbox:iten.type==0||iten.type==2||iten.type==3||iten.type==4,folderbox:iten.type==1}" @click="onSelectImg(iten)">
                          ...
                          <!--附件-->
                          <div class="img_raw" v-if="iten.type==4" >
                            <div class="img_preview">
                               <Icon icon="svgfont-zipyasuo" :size="45" v-if="iten.mimetype.startsWith('application/x-')"></Icon>
                               <Icon icon="svgfont-wps" :size="45" v-else-if="iten.mimetype.startsWith('application/vnd.')"></Icon>
                               <Icon icon="svgfont-Pdf" :size="45" v-else-if="iten.mimetype.startsWith('application/pdf')"></Icon>
                               <Icon icon="svgfont-100wanxuqiufabu" :size="45"  v-else></Icon>
                            </div>
                          </div>
                           <!--音频-->
                          <div class="img_raw" v-if="iten.type==3" >
                            <div class="img_preview">
                               <Icon icon="svgfont-yinpin" :size="45" ></Icon>
                            </div>
                          </div>
                          <!--视频-->
                          <div class="img_raw" v-else-if="iten.type==2" >
                            <div class="img_preview videobox">
                              <img :src="GetFullPath(iten.cover_url)" class="img_item"/>
                        ...
                        <div class="titlebox">
                          <div class="edit_title">
                            <a-input v-model="iten.title" placeholder="输入名称" :readonly="iten.is_common==1" style=" padding: 0;" @change="onChangFile" @pressEnter="(e:any)=>{onUpFile(iten.id,e)}" @blur="(e:any)=>{onUpFile(iten.id,e)}" :bordered="false"/> 
                          </div>
                        </div>
                      </div>
                    </div>
...
<script lang="ts">
  ...
  //api
  import { getMyFiles,upImgPid,delFile,delDir,saveFile,FileItem,PictureItem} from './api';
  import { userUploadadmin } from '@/api/common';
     ...
     const modelTime=ref("")
      ...
      const [registerModal, { setModalProps, closeModal }] = useModalInner(async (data) => {
          ...
          if(pagedata.selectdata.filetype=="video"){
            ...
            pagedata.modaldes=`(只能添加音频类文件x, 大小不超过${MaxSizeImage}MB。)`
            ...
            pagedata.modaldes=`(只能添加jpg,jpeg,bmp,png,gif.zip,.rar,.xls,.xlsx,.pdf,.doc,.docx, 大小不超过${MaxSizeImage}MB。)`
        }
        getMyfile("")
      });
        ...
        try {
          Message.loading({ content: '提交中...', id:"saveData",duration:0});
            ...
            if(resultdata){
              ...
              Message.success({ content: '创建成功！', id:"saveData", duration: 2000 });
          ...
          const controller = new AbortController();
            (async function requestWrap() {
              ...
              try {
                ...
                const resdata = await userUploadadmin({ name: 'file', file: fileItem.file as Blob, filename,data:{pid:pagedata.pid,filetype:pagedata.selectdata.filetype}},onUploadProgress);
                //更新图片
                if(resdata){
                  if(resdata["code"]==0){
