<template>
   <div class="navbar" :style="{ backgroundImage:(theme === 'dark'||!appStore.navBg)?'':'url(https://cms.goflys.cn/navbarbg.jpg) ', backgroundSize:'100% 60px'}">
    ...
    <ul class="right-side">
      <li>
        <a-tooltip :content="$t('navbar.businessIn')">
          <a-button @click="handleOpenBusiness" class="nav-btn" type="outline" shape="circle">
            <template #icon>
              <icon-export />
            </template>
          </a-button>
      ...
      <li>
        ...
        <a-dropdown trigger="hover">
          <a-row align="center" :wrap="false" class="user">
            <!-- 管理员头像 -->
            <a-avatar :size="32" :imageUrl="userStore.avatar"></a-avatar>
...
<script lang="ts" setup>
  ...
  const handleOpenBusiness=()=>{
     if(import.meta.env.VITE_APP_ENV=="production"){
       window.open(window?.globalConfig.Domain+"/webbusiness/",'_blank')
     }else{
      Message.warning("开发环境根据你运行端口号打开！")
     }
  }
</script>
