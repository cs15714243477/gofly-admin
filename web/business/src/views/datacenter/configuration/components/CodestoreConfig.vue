        ...
        <template #title>
         <div class="flex">
          <div style="flex:1;"> 
            ...
            <a-tooltip content="配置字段命名说明：_content结尾使用富文本编辑器，_txt结尾是多行文本（wxapp_property_form_options 已支持“选项编辑器”），_read结尾是只读，_switch结尾开关,#注释加 &des 是字段说明。" position="top" mini>
            ...
            <template v-for="item in ConfData.data" :key="item.keyfield">
              <a-col :span="getColSpan(item)">
                  ...
                  >
                    ...
                    <div v-else-if="isOptionsEditorField(item)" class="options-editor">
                    ...
                    <a-textarea v-else-if="item.keyfield.indexOf('_txt')>-1" v-model="item.keyvalue" :placeholder="`填写${FontNameAndDes(item.keyname,0)}`" :auto-size="{ minRows: 3, maxRows: 10 }"/>
