      ...
      <a-row :gutter="16">
        ...
        <a-col :span="12">
          <a-form-item field="url" label="接口路径" :rules="[{required:true,message:'请填写接接口路径'}]" >
            ...
            <a-select v-else  placeholder="请填接口路径" allow-search allow-clear @change="hanldeUrl">
        ...
        <a-col :span="12">
          <a-form-item field="tablename" label="数据表名" style="margin-bottom:15px;">
            <a-select v-model="formData.tablename" placeholder="请填数据表名(选填)" allow-search  allow-clear>
              <template #label="{ data }">
        ...
        <a-col :span="10" v-if="formData.method=='get'">
          <a-form-item field="getdata_type" label="获取数据类型" style="margin-bottom:5px;">
            <a-radio-group v-model="formData.getdata_type"  @change="handelGetdataType">
              ...
              <a-radio value="data">单条数据</a-radio>
            </a-radio-group>
          </a-form-item>
        </a-col>
          ...
          <a-col :span="24" v-if="formData.apicode_type==2">
            <div class="tigmsg"><icon-exclamation-circle />“通用数据接口”无需编写代码 直接调用通用api接口即可完成增删查改（传api_id区分）</div>
          </a-col>
        </template>
      </a-row>
    </a-form>
...
<script lang="ts">
  ...
  //api
  import { getGroup,save,DataItem,getRoutes} from './api/devapi';
  import { getTables,TableItem } from '@/api/common';
  import {installcode,uninstallcode,removeFile} from './api/apicode';
  ...
  export default defineComponent({
    ...
    setup(_, { emit }) {
      ...
      const basedata={
            ...
            fields:[],//查询字段
            getdata_type:"list",//获取数据类型list=多条，data=单条
        }
     ...
     //选择路由地址
     const hanldeUrl=(data:any)=>{
      if(data){
        const urlstr=data.split(":")
        if(urlstr.length>=1){
          formData.value.url=urlstr[1]
          if(urlstr[0]=="GET"){
            formData.value.method="get"
          }else if(urlstr[0]=="POST"){
            formData.value.method="post"
          }else if(urlstr[0]=="PUT"){
            formData.value.method="put"
          }else if(urlstr[0]=="DELETE"){
     ...
     const getMethod=()=>{
        ...
        if(formData.value.method=='get'){
          if(formData.value.getdata_type=="list"){
            mstr="getList"
          }else if(formData.value.getdata_type=="data"){
            mstr="getData"
          }
        }else if(formData.value.method=='post'){
           mstr="add"
        }else if(formData.value.method=='put'){
           mstr="put"
