<template>
  <BasicModal v-bind="$attrs" @register="registerModal" :loading="loading" :footer="false" helpMessage="接口分类" width="900px" :minHeight="600" :title="getTitle" @ok="handleSubmit">
    <a-row style="margin-bottom: 1px">
        ...
        <div class="talbe-modal">
          <a-table
              ...
              >
                ...
                <template #operations="{ record }">
                  <Icon icon="svgfont-edit-o" class="iconbtn" @click="handleTypeEdit(record)" :size="18" color="#0960bd"></Icon>
                  <a-divider direction="vertical" />
      ...
      <!--添加类型-->
      <a-modal v-model:visible="AddTypeVisible" @cancel="AddTypeVisible=false" width="700px" @ok="handleTypeSubmit" :closable="false">
        <a-form ref="typeformRef" :model="typeForm" auto-label-width>
          ...
          <template v-if="typeForm.isself==0" >
            ...
            <a-form-item field="verifyencrypt" label="请求验证字符串" validate-trigger="input" style="margin-bottom:15px;">
...
<script lang="ts">
  ...
  //api
  import { getParent,getList, save,del} from './api/devapigroup';
  import { getType, saveType,delType} from './api/devapitype';
  import { getRoutes,getModel,getTablelist,UserItem} from './api/devapi';
