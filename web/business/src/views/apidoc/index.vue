<template>
  <div class="container" >
      <page-card breadcrumb scrollPage ref="onelineCardRef" >
         <div class="devapi">
           <div class="left">
            <a-row style="margin-bottom: 10px">
              ...
              <a-space>
                ...
                :fieldNames="{
                    ...
                    title: 'name',
                    children: 'children'
                  }"  v-model="searchData.cid" @change="handleData">
                </a-tree-select>
          ...
          <div class="tablebox">
           ...
           <div class="tigbox">
              ...
              <div class="item">
                ...
                <div class="text">{{ userInfo.$state.business_id }}</div>
              </div>
              <div class="item">
              ...
              <div class="item">
                <div class="label">3.接口请求验证，请求头添加 apiverify=base64(md5(时间戳+接口验证字符串)),接口验证字符串在<span style="color: #165dff;">管理分组->接口配置</span>中</div>
                <div class="text"></div>
              </div>
              <div class="item">
  ...
  <script lang="ts">
   ...
   import { useClipboard } from '@vueuse/core';
    ...
    //api
    import { getList,del,getGroup,getDBField,getToken} from './api/devapi';
    import { getTypeinfo } from './api/devapitype';
    //table
      ...
      setup() {
        ...
        const handleSelect=(record: Recordable)=>{
          ...
          if(record.apicode_type==2){
            gerurl=cloneDeep(`${gerurl}?api_id=${record.api_id}`)
          }
        ...
        async function otherHttp(){
          ...
          let baseRootUrl = import.meta.env.VITE_APP_ENV=="production"? typeInfo.value.rooturl: window?.globalConfig.Main_url_dev;//前缀
          //获取token
          let tokendata=""
         ...
         const passstr=md5(typeInfo.value.verifyencrypt+timestamp) as any
         const apiverifydata=window.btoa(passstr+"#"+timestamp);//MD5加密后#拼接时间戳
          if(requestData.value.method=="get"){
              return axios.get(baseRootUrl+requestData.value.url, { params: params,headers: {'apiverify': apiverifydata,"Authorization":tokendata,"Businessid":userInfo.$state?.business_id+""} }).catch(err => {
                  requestAfter.value.laoding="error"
              ...
              return axios.post(baseRootUrl+requestData.value.url, params,{headers: {'apiverify': apiverifydata,"Authorization":tokendata,"Businessid":userInfo.$state?.business_id+""}}).catch(err => {
                  requestAfter.value.laoding="error"
                  requestAfter.value.message=err.message
                  requestAfter.value.status=err.response.status
                  requestAfter.value.result=err.response.data.message
              });
            }else if(requestData.value.method=="put"){
              return axios.put(baseRootUrl+requestData.value.url, params,{headers: {'apiverify': apiverifydata,"Authorization":tokendata,"Businessid":userInfo.$state?.business_id+""}}).catch(err => {
                  requestAfter.value.laoding="error"
                  requestAfter.value.message=err.message
                  requestAfter.value.status=err.response.status
                  requestAfter.value.result=err.response.data.message
              ...
              return axios.delete(baseRootUrl+requestData.value.url,{ data: params,headers: {'apiverify': apiverifydata,"Authorization":tokendata,"Businessid":userInfo.$state?.business_id+""} }).catch(err => {
                  requestAfter.value.laoding="error"
                  requestAfter.value.message=err.message
                  requestAfter.value.status=err.response.status
                  requestAfter.value.result=err.response.data.message
        ...
        function reqhttp(){
          var params={}
          requestAfter.value.result=""
          if(requestData.value.param){
            params= eval('(' + requestData.value.param + ')');
            // params=JSON.parse(requestData.value.param)
          }
          ...
          if(requestData.value.method=="get"){
              ...
              return defHttp.put({ url: requestData.value.url, params:params}, { isReturnNativeResponse: true}).catch(err => {
                  requestAfter.value.laoding="error"
                  requestAfter.value.message=err.message
                  requestAfter.value.status=err.response.status
                  requestAfter.value.result=err.response.data.message
