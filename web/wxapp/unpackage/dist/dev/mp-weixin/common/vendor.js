...
const toDisplayString = (val) => {
  return isString(val) ? val : val == null ? "" : isArray$5(val) || isObject$7(val) && (val.toString === objectToString$2 || !isFunction$3(val.toString)) ? JSON.stringify(val, replacer, 2) : String(val);
};
const replacer = (_key, val) => {
  ...
  } else if (isObject$7(val) && !isArray$5(val) && !isPlainObject$2(val)) {
    return String(val);
  }
  return val;
};
...
function createInvoker(initialValue, instance) {
  const invoker = (e2) => {
    ...
    if (bubbles.includes(e2.type) && !eventSync) {
      ...
      const res = invoke();
      if (e2.type === "input" && (isArray$5(res) || isPromise(res))) {
        return;
      }
      return res;
    }
...
function removeInterceptorHook(interceptors2, interceptor) {
  ...
  Object.keys(interceptor).forEach((name) => {
    ...
    if (isArray$5(hooks) && isFunction$3(hook)) {
      remove(hooks, hook);
    }
  });
}
function mergeHook(parentVal, childVal) {
  const res = childVal ? parentVal ? parentVal.concat(childVal) : isArray$5(childVal) ? childVal : [childVal] : parentVal;
...
function initRuntimeSocketService() {
  const hosts = "192.168.110.1,192.168.71.1,192.168.78.103,127.0.0.1,172.23.64.1,172.24.32.1";
  const port = "8090";
  const id = "mp-weixin_Mleaip";
...
function initBaseInstance(instance, options) {
  ...
  instance.slots = {};
  if (isArray$5(options.slots) && options.slots.length) {
...
function parsePropType(type, defaultValue) {
  if (isArray$5(type) && type.length === 1) {
    return type[0];
  }
  return type;
}
...
function initFormField(vm) {
  ...
  if (isArray$5(vueOptions.behaviors) && vueOptions.behaviors.includes("uni://form-field")) {
    vm.$watch("modelValue", () => {
...
function initBehaviors(vueOptions) {
  ...
  const behaviors = [];
  if (isArray$5(vueBehaviors)) {
    vueBehaviors.forEach((behavior) => {
      behaviors.push(behavior.replace("uni://", "wx://"));
      if (behavior === "uni://form-field") {
        if (isArray$5(vueProps)) {
          vueProps.push("name");
...
function parseComponent(vueOptions, { parse, mocks: mocks2, isPage: isPage2, isPageInProject, initRelation: initRelation2, handleLink: handleLink2, initLifetimes: initLifetimes2 }) {
  ...
  if (isArray$5(vueOptions.mixins)) {
    vueOptions.mixins.forEach((item) => {
      if (isObject$7(item.options)) {
