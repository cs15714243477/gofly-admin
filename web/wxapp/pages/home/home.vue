	...
	<view class="home-container">
		...
		<scroll-view scroll-y="true" class="main-content">
			...
			<view class="top-tools">
				<view class="top-tools__inner">
					...
					<view class="city-selector" @click="selectProvince">
						...
						<text class="material-symbols-outlined expand-icon">expand_more</text>
					</view>
					<view class="search-box">
						<text class="material-symbols-outlined search-icon">search</text>
						<input class="search-input" type="text" placeholder="搜索区、商圈、街道..." placeholder-class="placeholder" />
					</view>
					<!-- 更多筛选：移到搜索栏右侧，避免下方文字拥挤 -->
					<view class="more-btn" :class="{ active: filters.more.length > 0 }" @click="openPanel('more')">
			...
			<view class="filter-bar">
				<!-- 市 / 区：左侧二级联动选择 -->
				<view class="filter-item" :class="{ active: !!filters.location }" @click="openLocationPicker">
					<text class="filter-text">{{ filterLabelLocation }}</text>
					<text class="material-symbols-outlined filter-icon">expand_more</text>
				</view>
				<view class="filter-item" :class="{ active: !!filters.filter }" @click="openPanel('filter')">
					<text class="filter-text">{{ filterLabelFilter }}</text>
					<text class="material-symbols-outlined filter-icon">expand_more</text>
				</view>
				<view class="filter-item" :class="{ active: !!filters.sort }" @click="openPanel('sort')">
			...
			<view class="section">
				<view class="section-header">
					<view class="section-title">
						<text class="material-symbols-outlined section-icon accent">favorite</text>
				...
				<scroll-view scroll-x="true" class="horizontal-scroll">
					<view class="property-card-h" v-for="(item, index) in followedProperties" :key="index" @click="goToDetail(item)">
						<view class="card-image" :style="{ backgroundImage: 'url(' + item.image + ')' }">
							<view class="image-tag">{{ item.type }}</view>
							<view class="price-change-tag" v-if="item.priceChange">{{ item.priceChange }}</view>
							<view class="status-tag" v-if="item.status">{{ item.status }}</view>
						</view>
						<view class="card-info">
							<view class="property-name">{{ item.name }}</view>
							<view class="property-meta">
								<text>{{ item.area }} • {{ item.size }}㎡</text>
								<view class="price">
			...
			<view class="section">
				<view class="section-header">
					<view class="section-title">
						<text class="material-symbols-outlined section-icon primary">recommend</text>
						<text class="title-text">系统推荐的房源</text>
					</view>
				</view>
				<view class="vertical-list">
					<view class="property-card-v" v-for="(item, index) in recommendedProperties" :key="index" @click="goToDetail(item)">
						<view class="card-image-v" :style="{ backgroundImage: 'url(' + item.image + ')' }">
							<view class="status-tag-v" v-if="item.status">{{ item.status }}</view>
						</view>
						<view class="card-info-v">
							...
							<view class="property-location">
								<text class="material-symbols-outlined loc-icon">location_on</text>
								<text>{{ item.region }} • {{ item.size }}㎡ • {{ item.desc }}</text>
							</view>
							<view class="tags">
								<text class="tag-item" v-for="(tag, tIdx) in item.tags" :key="tIdx" :class="{ 'tag-orange': tag === '新上' }">{{ tag }}</text>
							</view>
						</view>
					</view>
				</view>
			...
			<view class="sheet" @click.stop>
				...
				<view v-if="panelType !== 'more'" class="sheet-list">
					...
					>
						...
						<text v-if="isOptionActive(opt)" class="material-symbols-outlined sheet-check">check</text>
					</view>
				</view>
				...
				<view v-else class="sheet-list">
					...
					>
						<text class="sheet-item-text">{{ opt.label }}</text>
						<text v-if="isMoreSelected(opt)" class="material-symbols-outlined sheet-check">check</text>
					</view>
				</view>
		...
		<view v-if="locationOpen" class="loc-mask" @click="closeLocationPicker">
			<view class="loc-drawer" @click.stop>
				...
				<view class="loc-foot">
					<button class="loc-btn ghost" @click="closeLocationPicker">取消</button>
					<button class="loc-btn primary" @click="applyLocationPicker">确定</button>
				</view>
			</view>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			isOptionActive(opt) {
				if (!opt) return false
				if (this.panelType === 'location') return this.panelDraft.location === opt.value
				if (this.panelType === 'filter') return this.panelDraft.filter === opt.value
