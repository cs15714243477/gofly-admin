...
var (
	dbprefix = String(dbConf_arr["prefix"])
)
...
func removePrefix(table, prefix string) string {
	return strings.TrimLeft(table, prefix)
}
...
func writeTableStruct(db *sql.DB, table string, buf *bufio.Writer) error {
	...
	if createTableSQL != "" {
		createTableSQL = strings.ReplaceAll(createTableSQL, "NOT EXISTS `"+dbprefix, "NOT EXISTS `")
	}
...
func writeTableData(db *sql.DB, table string, buf *bufio.Writer) error {
	...
	for _, row := range values {
		ssql := "INSERT INTO `" + removePrefix(table, dbprefix) + "` VALUES ("
